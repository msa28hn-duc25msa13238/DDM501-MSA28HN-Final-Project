...
Tài liệu này mô tả **luồng hoàn chỉnh**: huấn luyện, API, giám sát Prometheus/Grafana, alerting, báo cáo Evidently, Responsible AI, kiểm thử tải, pre-commit và CI. Chi tiết kiến trúc: [ARCHITECTURE.md](./ARCHITECTURE.md). Monitoring vận hành: [MONITORING.md](./MONITORING.md). Responsible AI: [RESPONSIBLE_AI.md](./RESPONSIBLE_AI.md). Mở rộng kỹ thuật: [PIPELINE_EXTENSIONS.md](./PIPELINE_EXTENSIONS.md).
...
Sau khi chạy pipeline huấn luyện, thư mục `models/responsible_ai/` sẽ được tạo
tự động với các artifact:

- `fairness_group_metrics.csv`: RMSE / MAE / WAPE theo `state_id`, `store_id`, `cat_id`, `dept_id`, `item_id`
- `fairness_summary.json`: nhóm tốt nhất / kém nhất theo MAE và độ chênh lệch
- `explainability_permutation_importance.csv`: mức độ quan trọng của từng feature
- `explainability_summary.json`: tóm tắt explainability
- `explainability_top_features.png`: biểu đồ top feature

`GET /model/info` cũng trả về phần `responsible_ai` từ model bundle để có thể
kiểm tra nhanh sau khi deploy API.
...
## 10. Checklist smoke test trước demo
...
5. Kiểm tra `models/responsible_ai/` có đủ CSV / JSON / PNG cho fairness và explainability.
6. (Tuỳ chọn) `python -m scripts.evidently_report --demo` tạo HTML trong `reports/`.
